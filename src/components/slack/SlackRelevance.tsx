...
export function SlackRelevance({
  ...
  const [mode, setMode] = useState<"skill" | "activity">("skill");
    ...
    startTransition(async () => {
      try {
        const res = await fetch("/api/slack/search", {
          ...
          body: JSON.stringify({ mode, id: Number(id) }),
        });
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-5 mb-6">
      ...
      <p className="text-xs text-gray-500 mb-3">
        Pick a skill or activity and we&apos;ll surface Slack posts from
        your connected channels that match it.
      </p>
